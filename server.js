...
// Download campaign variations as ZIP file
app.post('/api/download-campaign', async (req, res) => {
  try {
    ...
    const filename = `${campaign.replace(/[^a-z0-9]/gi, '_')}_${timestamp}.zip`;
    ...
    for (const variation of variations) {
      ...
      // Decode base64 image
      const imageBuffer = Buffer.from(variation.imageUrl, 'base64');

      // Add image to ZIP with organized folder structure
      archive.append(imageBuffer, {
        name: `${marketName}/${marketName}_campaign.jpg`
      });

      // Add campaign details as text file
      const details = `Campaign: ${campaign}
Industry: ${industry}
Market: ${marketName}
Cultural Insight: ${variation.culturalInsight}
...
`;
      ...
      archive.append(details, {
        name: `${marketName}/${marketName}_details.txt`
      });
    }

    // Add a README file
    const readme = `Market Localizer Campaign Export
Generated by AI-powered Market Localizer
Powered by Google Imagen 4

Campaign: ${campaign}
Industry: ${industry}
Markets: ${variations.map(v => v.market).join(', ')}

Generated: ${new Date().toISOString()}

Each folder contains:
- Campaign image variation for that market
- Details text file with cultural insights and prompt
...
`;
    ...
    console.log('Campaign ZIP download completed');
